    ...
    with torch.no_grad():
        ...
        for data in tqdm(test_loader):
            ...
            metric_i = test_single_volume(
                ...
                test_save_path=None, case=case_name, z_spacing=config.z_spacing, val_or_test=True
            )
            metric_list += np.array(metric_i)

            img_np = img.squeeze(0).cpu().detach().numpy()
            x, y = img_np.shape[-2], img_np.shape[-1]
            input_tensor = img.cuda().float()
            outputs = model(input_tensor)
            pred = torch.argmax(torch.softmax(outputs, dim=1), dim=1)

            save_pred_imgs(pred, i_batch, test_save_path, threshold=threshold, test_data_name=case_name)
        ...
        for i in range(1, config.num_classes):
            logger.info('Mean class %d mean_dice %f mean_hd95 %f' % (i, metric_list[i-1][0], metric_list[i-1][1]))
        etime = time.time()
        log_info = f'Test finished, mean_dice: {performance}, mean_hd95: {mean_hd95}, time(s): {etime-stime:.2f}'
        print(log_info)
        logger.info(log_info)
    return performance, mean_hd95