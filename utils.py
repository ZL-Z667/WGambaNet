...
def save_pred_imgs(msk_pred, img_name, save_path, threshold=0.5, test_data_name=None):
    """
    msk_pred shape: [1, H, W] OR [H, W]
    img_name: e.g. "xxx.png"
    """
    ...
    if test_data_name is not None:
        save_path = os.path.join(save_path, test_data_name + '_')
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    plt.savefig(os.path.join(save_path, img_name), bbox_inches='tight', pad_inches=0)
    plt.close()
'''
def save_pred_imgs(msk_pred, i, save_path, threshold=0.5, test_data_name=None):
    ...
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    plt.savefig(os.path.join(save_path, f"{i}.png"), bbox_inches='tight', pad_inches=0)
    plt.close()
'''
...
class myNormalize:
    def __init__(self, data_name, train=True):
        if data_name == 'isic18' or 'isic18zyr':
            if train:
                self.mean = 157.561
                self.std = 26.706
            else:
...
def calculate_acdc_metrics(pred, gt, num_classes):
    metrics = {
        'DSC': 0.0,
        'RV': 0.0,
        'MYO': 0.0,
    ...
    for class_id in range(1, num_classes):
        ...
        if class_id == 1:
            ...
            metrics['LV'] = dice
    metrics['DSC'] = np.mean(dice_list)
    metrics['HD95'] = np.mean(hd95_list)
    return metrics


'''
def test_single_volume_acdc(image, label, net, classes, patch_size, test_save_path, case, z_spacing, save_slices_dir=None):
    ...
    for slice_idx in range(num_slices):
        ...
        if slice_img.shape != patch_size:
            slice_img = zoom(slice_img, (patch_size[0] / original_shape[0], patch_size[1] / original_shape[1]), order=3)
        ...
        if pred_slice.shape != original_shape:
            pred_slice = zoom(pred_slice, (original_shape[0] / patch_size[0], original_shape[1] / patch_size[1]), order=0)
    ...
    metrics = calculate_acdc_metrics(prediction, label, classes)
